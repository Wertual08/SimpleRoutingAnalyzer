...
{
    class RestrictedGreedyPromotion2Routing : IRoutingAlgorithm
    {
        public static readonly string Name = "Restricted Greedy Promotion 2";

        private GreedyPromotionData Data;
        private Graph Graph;

        public RestrictedGreedyPromotion2Routing(Graph graph)
        {
        ...
        public int[] Route(RoutingData data)
        {
            ...
            {
                if (!Data.IsExceptional(dest) && dest != d && Graph.Available(dest) <= 2) continue;
                total_available++;
